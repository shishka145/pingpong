...
# Creating logo
logo = pygame.transform.scale(pygame.image.load("ping pong logo.png"), (64, 64))
...
# Background image
background = pygame.image.load("backgroung.jpeg")
background = pygame.transform.scale(background, (800, 600))

# Player A
player_a_img = pygame.transform.scale(pygame.image.load("bar.png"), (32, 128))
player_a_x_cor = 0
player_a_y_cor = 234
...
# Player B
player_b_img = pygame.transform.scale(pygame.image.load("bar.png"), (32, 128))
player_b_x_cor = 768
player_b_y_cor = 234
...
# Ball
ball_img = pygame.transform.scale(pygame.image.load("Ball.png"), (50, 50))
ball_x_cor = 350
ball_y_cor = 250
...
def show_score():
    ...
    window.blit(pygame.font.Font("freesansbold.ttf", 40).render(f"Player A : {player_a_score}", True, (0, 0, 0)),
                (10, 10))
    window.blit(pygame.font.Font("freesansbold.ttf", 40).render(f"Player B : {player_b_score}", True, (0, 0, 0)),
                (410, 10))
...
while running:
    ...
    if ball_y_cor <= -5:
        ball_y_cor = -5
        ball_y_cor_change *= -1
        pygame.mixer.Sound("wall.wav").play()  # Wall hit sound
    ...
    if (0 <= ball_x_cor <= 32) and (ball_y_cor + 45 >= player_a_y_cor and ball_y_cor - 5 <= player_a_y_cor + 128):
        ball_x_cor = 33
        ball_x_cor_change *= -1
        pygame.mixer.Sound("ball hit paddle.wav").play()  # Sound when ball hits player
    ...
    if ball_x_cor < 0:
        ...
        pygame.mixer.Sound("ball missed.wav").play()  # Sound when ball missed by player
